...
bool WFG1::init (int nObj, int k, int l) {
	this->nObj = nObj;
	if (nObj < 2) {
...
//	cout<< "objs: "<< nObj <<"valorrrr " << D*x.back() << endl;
   this->Xh = x.back();
  return result;
}
...
vector< double > WFG1::WFG_1_random_soln( const int k, const int l, vector<double> &spacek )
{
  ...
  for( int i = 0; i < k; i++ )
  {
    ...
    result.push_back( pow( spacek[i], 50.0 ) );
    //result.push_back( pow( (double)rand()/RAND_MAX, 50.0 ) );
  }
